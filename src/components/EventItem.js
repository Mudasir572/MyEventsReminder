...
export default function EventItem(props) {
  ...
  const { title, discription, type, date,startTime,endTime,id,attachment,setChangeHappen,changeHappen } = props;
  ...
  const updateEventForm = (e) =>{
    navigate('/updateevent',{state: {eventTitle: title,eventDiscription: discription,eventType: type,eventDate: date,eventStart: startTime,eventEnd: endTime,eventId: id,eventAttachment: attachment}})
  } 
    ...
    <div className="my-3 col-md-4" >
      <div className="card" style={{backgroundColor: "#d5e4f3"}}>
        <div className="card-body">
          ...
          <button type="button" data-eventid={id} onClick={updateEventForm} className="btn btn-sm btn-primary">
            Update
          </button>
          <button type="button" data-eventid={id} onClick={deleteEvent} className="btn btn-sm btn-danger mx-3">
            Delete
          </button>
          {attachment && attachment !== "" ? <a href=""  onClick={getAttachedFile}>
          <svg 
           xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-filetype-pdf" viewBox="0 0 16 16">
